...
class NoiseNode(CtrlNode):
    """Node that produces spurious noise"""
    nodeName = 'Noise'
    #NOTE: This node works only on toggling the 'wave mode' button

    uiTemplate = [
        ('wave mode', 'combo', {'values': ['even', 'odd'], 'index': 9})
    ]
    ...
    def process(self, display= True):
        data = []
        x = random.randrange (0,9)#random should have 9 values, 1 for each of the bit position and 1 for no noise         
        for i in range(0,8):
            data.append(0)  
        if  x < 8: 
            data[x] = 1

        out_data = metaarray.MetaArray(data, info=[{'name': 'Time', 'values': np.linspace(0, 8, 8)}, {}])
        
        return {'gOut': out_data}
    


class DigiAdderNode(Node):
    """Node that digitally adds bits given as input"""
    nodeName = 'DigiAdder'

    uiTemplate = [
        ('wave mode', 'combo', {'values': ['even', 'odd'], 'index': 0})
    ]
    ...
    def process(self, In1, In2, display=True):
        ...
        for i in range(0,8):
            temp_list1.append(data1.item(i)) #actual code, ok this is working!
            #temp_list1.append(data1.item(0))  # testing code
            #temp_list1.append(0)  # testing code - does the block work?, ok it works
             
        for i in range(0,8):
            temp_list2.append(data2.item(i)) #actual code
            #temp_list2.append(data2.item(0))  # testing code
            #temp_list2.append(1)  # testing code
            
        for i in range(0,8):
            if (temp_list1[i] == 0 and temp_list2[i] == 0):
                #output[i] = 0  # man, this was the culprit
                output.append(0)
            else:
                #output[i] = 1
                output.append(1)

        #for i in range(0,8):  #code to test if output work                
            #output.append(1) ...... this code works

        #if output == [1,1]:  #this stage was used to test the expected output when oscillator wasn't displaying any output
...
comments:
1. Node comes alive on display but without input or output notches!
No way to connect the node. What did I miss?!
2. Now node works but no output displayed.
'''
